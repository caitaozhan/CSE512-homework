...
% Last modified: 21-Dec-2015    
    ...
    methods
        ...
        % isEndAlign: 
        %   default 0. If isEndAlign, the last window considered will have a corner at the 
        %   bottom right of the image (data matrix for multi-dim).
        %   If isEndAlign = 0, the first window will have a corner at the top left of the data
        %   matrix. The stepSz might not be devisible by the dimension so the last window might
        %   not contain the bottom right corner of the data matrix. This option allows us to
        %   specify our preference: (1) start from the top left, or (2) end at the bottom right
        %   At least one condition will be satisfied, and sometimes both will be satisfied. 
        function obj = ML_SlideWin(Data, winSz, stepSz, memLimit, isEndAlign)
            % Sanity check
            k = ndims(Data);
            ...
            for i=1:k
                ...
                if exist('isEndAlign', 'var') && isEndAlign
                    startIdxs{i} = fliplr((size(Data,i) - winSz(i)+1):-stepSz(i):1);
                else
...
%             end;
                ...
                % This code is twice as fast as using repmat above
                d = length(self.winLinIdxs);
                k = length(self.batchLinIdxs{batchId});
                batchIdxs = self.batchLinIdxs{batchId};
                D = zeros(d, k);
                if d > k
                    for i=1:k
